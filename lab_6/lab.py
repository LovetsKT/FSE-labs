...
def visualize_all_charts(user_stats, workouts):
    ...
    for workout in workouts:
        workout_type = workout['type']
        efficiency = workout['calories'] / workout['duration']
    ...
    avg_efficiencies = {t: d['total'] / d['count'] for t, d in efficiency_data.items()}

    names = list(user_stats.keys())
    calories = [stats['total_calories'] for stats in user_stats.values()]
    workout_counts = [stats['total_workouts'] for stats in user_stats.values()]

    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('ВИЗУАЛИЗАЦИЯ ДАННЫХ ФИТНЕС-ТРЕНЕРА', fontsize=18, fontweight='bold')

    axes[0, 0].pie(type_counts.values(), labels=type_counts.keys(),
                   autopct='%1.1f%%', startangle=90, colors=plt.cm.Set3(range(len(type_counts))))
    axes[0, 0].set_title('Распределение типов тренировок')
    axes[0, 0].axis('equal')

    bars1 = axes[0, 1].bar(names, workout_counts, color='skyblue', edgecolor='black')
    axes[0, 1].set_title('Активность пользователей')
    ...
    eff_types = list(avg_efficiencies.keys())
    eff_values = list(avg_efficiencies.values())
    bars2 = axes[1, 0].bar(eff_types, eff_values, color=['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4'][:len(eff_types)])
    axes[1, 0].set_title('Эффективность тренировок (ккал/мин)')
    axes[1, 0].set_xlabel('Тип тренировки')
    axes[1, 0].set_ylabel('Калорий в минуту')

    color_map = {'продвинутый': 'red', 'средний': 'orange', 'начальный': 'green'}
    bar_colors = [color_map.get(user_stats[name]['fitness_level'], 'blue') for name in names]
    bars3 = axes[1, 1].bar(names, calories, color=bar_colors, edgecolor='black')
    axes[1, 1].set_title('Сравнение по затраченным калориям')
    axes[1, 1].set_xlabel('Пользователи')
    axes[1, 1].set_ylabel('Затраченные калории')
